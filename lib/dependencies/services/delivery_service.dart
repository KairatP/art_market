...
abstract class ArtDeliveryService {
  ...
  Future<ArtModel> getArtList(int pageNumber, int pageSize, String search);
...
class ArtListServiceImplement implements ArtDeliveryService {
  ...
  @override
  Future updateMyProfile(File ava, String name, String phoneNumber,
    ...
    if (ava.path.isNotEmpty) {
      sendData = FormData.fromMap({
        ...
        "country": country,
        "city": city,
        "avatar": await MultipartFile.fromFile(ava.path,
            filename: ava.path.split("/").last),
      });
    } else {
      sendData = FormData.fromMap({
        "name": name,
    ...
    // var avaImage = await MultipartFile.fromFile(ava.path,
    //     filename: ava.path.split("/").last);
    try {
      Response response =
          await dio.post('account/updateProfile', data: sendData);
  ...
  Future deletePost(String deletePost) async {
    ...
    try {
      print(deletePost);
      Response response = await dio.delete('post/delete', data: {
        "id": deletePost,
      });
